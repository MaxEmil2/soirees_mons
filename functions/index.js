...
// Configurer la région par défaut pour toutes les fonctions
    ...
    try {
        ...
        if (userDoc.exists && userDoc.data().stripeAccountId) {
            ...
            const accountLink = await stripeClient.accountLinks.create({
                ...
                refresh_url: `${baseUrl}/dashboard.html?stripe=refresh`,
                return_url: `${baseUrl}/dashboard.html?stripe=success`,
...
exports.createCheckoutSession = onCall(async (request) => {
    if (!request.auth) {
        throw new HttpsError('unauthenticated', 'Vous devez être connecté pour acheter une prévente');
    }

    const {eventId, baseUrl, buyerInfo} = request.data;
    const userId = request.auth.uid;
    const userEmail = request.auth.token.email;
    ...
    try {
        ...
        if (!eventDoc.exists) {
            throw new HttpsError('not-found', 'Événement non trouvé');
        }
        ...
        if (event.status !== 'approved') {
            throw new HttpsError('failed-precondition', 'Cet événement n\'est pas encore approuvé');
        }

        if (!event.presales) {
            throw new HttpsError('failed-precondition', 'Les préventes ne sont pas activées pour cet événement');
        }

        // Vérifier si les préventes ont été stoppées manuellement
        if (event.presalesStopped) {
            throw new HttpsError('failed-precondition', 'Sold out ! Les préventes ont été arrêtées.');
        }

        // Vérifier si sold out (quota atteint)
        if (event.maxPresales && event.presalesSold >= event.maxPresales) {
            throw new HttpsError('failed-precondition', 'Sold out ! Toutes les préventes ont été vendues.');
        }
        ...
        if (event.presalesEndDate) {
            ...
            if (new Date() > endDate) {
                throw new HttpsError('failed-precondition', 'Les préventes sont terminées pour cet événement');
            }
        }
        ...
        if (!creatorDoc.exists || !creatorDoc.data().stripeAccountId) {
            throw new HttpsError('failed-precondition', 'Le créateur de l\'événement n\'a pas configuré son compte de paiement');
        }
...
async function sendPresaleEmail(presaleData) {
    ...
    const msg = {
        ...
        from: {
            email: process.env.SENDGRID_FROM_EMAIL || 'noreply@soireesmons.be',
            name: 'Soirées Mons'
        },
...
exports.verifyTicket = onCall(async (request) => {
    ...
    const {qrCodeData, eventId} = request.data;
 ...
 */
exports.markTicketUsed = onCall(async (request) => {
    if (!request.auth) {
        throw new HttpsError('unauthenticated', 'Vous devez être connecté');
    }

    const {presaleId, eventId} = request.data;
    ...
    try {
        ...
        if (event.createdBy !== scannerId && !isAdmin && !isScanner) {
            throw new HttpsError('permission-denied', 'Vous n\'êtes pas autorisé à scanner les tickets pour cet événement');
        }

        // Récupérer et vérifier la prévente
        const presaleDoc = await db.collection('presales').doc(presaleId).get();
        if (!presaleDoc.exists) {
            throw new HttpsError('not-found', 'Prévente non trouvée');
        }

        const presale = presaleDoc.data();

        if (presale.eventId !== eventId) {
            throw new HttpsError('failed-precondition', 'Ce ticket n\'est pas pour cet événement');
        }

        if (presale.status !== 'valid') {
            throw new HttpsError('failed-precondition', `Ce ticket ne peut pas être utilisé (statut: ${presale.status})`);
        }
 ...
 */
exports.getPresalesForEvent = onCall(async (request) => {
    if (!request.auth) {
        throw new HttpsError('unauthenticated', 'Vous devez être connecté');
    }
 ...
 */
exports.getMyPresales = onCall(async (request) => {
    if (!request.auth) {
        throw new HttpsError('unauthenticated', 'Vous devez être connecté');
    }
 ...
 */
exports.getAllPresales = onCall(async (request) => {
    if (!request.auth) {
        throw new HttpsError('unauthenticated', 'Vous devez être connecté');
    }
 ...
 */
exports.refundPresale = onCall(async (request) => {
    if (!request.auth) {
        throw new HttpsError('unauthenticated', 'Vous devez être connecté');
    }

    const {presaleId} = request.data;
    const userId = request.auth.uid;
    ...
    try {
        ...
        if (presale.status === 'refunded') {
            throw new HttpsError('failed-precondition', 'Cette prévente a déjà été remboursée');
        }

        if (presale.status === 'used') {
            throw new HttpsError('failed-precondition', 'Impossible de rembourser un ticket déjà utilisé');
        }
...
exports.cleanupUsedPresales = onSchedule({
    schedule: '0 3 * * *',
